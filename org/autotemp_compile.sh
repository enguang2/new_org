...
if [ "$algo" = "advi" ]; then
    ...
    ls {${output_dir}/${org_prog}_robust_*/,${output_dir}/${org_prog}/} -d | grep -v "robust_local" | xargs -P 1 -n 1 -I {} sh -c "echo {};./attack_run_metric_res_compile.sh {} ${attack} ${metric_m} ${datagood}"
    #done
    echo "original result:"
    cat ${output_dir}/${org_prog}/*${attack}*/*all*vb_$metric_m

    for ff in `ls ${output_dir}/${org_prog}_robust_*/*${attack}*/*all*vb_$metric_m | grep -v "robust_local"`; do
        parsss=${ff#*_robust_}
    ...
    ls {${output_dir}/${org_prog}_robust_*/,${output_dir}/${org_prog}/} -d | grep -v "robust_mix\|robust_local" | xargs -P 1 -n 1 -I {} sh -c "echo {};./attack_run_metric_res_compile.sh {} ${attack} ${metric_m} ${datagood}"
    #done
    echo "original result:"
    cat ${output_dir}/${org_prog}/*${attack}*/*all*diff2_$metric_m

    for ff in `ls ${output_dir}/${org_prog}_robust_*/*${attack}*/*all*diff2_$metric_m | grep -v "robust_mix\|robust_local"`; do
        parsss=${ff#*_robust_}
        echo "robust ${parsss%%/*} result:"
        cat $ff
    done
